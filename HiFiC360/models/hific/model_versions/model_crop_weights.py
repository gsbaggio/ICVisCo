...
Nodes = collections.namedtuple(
    ...
    ["input_image",             # [0, 255]
     ...
     "spatial_weight"])         # Weight based on latitude.
...
class HiFiC(object):
  ...
  def build_input(self,
    ...
    if self._setup_discriminator:
      ...
      def _batch_to_dict(batch):
        ...
        spatial_weight, _ = tf.split(weights, splits)
      ...
      def _batch_to_dict(batch):
        return dict(input_image=batch["image"], spatial_weight=batch["weight"])
  ...
  def _get_dataset(self, batch_size, crop_size,
    ...
    with tf.name_scope("tfds"):
      ...
      def _preprocess(features):
        ...
        if not crop_size:
          return {"image": image, "weight": tf.constant(1.0)}
        ...
        with tf.name_scope("random_crop_weighted"):
          # Manual crop to calculate weight
          ...
          image = tf.slice(image, [offset_y, offset_x, 0], [crop_size, crop_size, 3])
          
          y_center = tf.cast(offset_y, tf.float32) + (crop_size_float / 2.0)
          h_float = tf.cast(img_h, tf.float32)
          
          phi = (3.14159265 / 2.0) - (y_center / h_float) * 3.14159265
          crop_weight = tf.cos(phi)
          
        return {"image": image, "weight": crop_weight}
  ...
  def build_model(self, input_image, input_images_d_steps=None, spatial_weight=None):
    ...
    Args:
      ...
      input_images_d_steps: If training a discriminator, this is expected to
        ...
        See build_input.
      spatial_weight: Tensor (B,) with weights for each image in batch.
    ...
    nodes_gen, bpp_pair, bitstrings = \
      self._compute_compression_graph(input_image, spatial_weight=spatial_weight)
  ...
  def _compute_compression_graph(self, input_image, spatial_weight=None, create_summaries=True):
    """Compute a forward pass through encoder and decoder.

    Args:
      input_image: Input image, range [0, 255]
      spatial_weight: Tensor (B,) with weights.
  ...
  def _compute_distortion_loss(self, nodes: Nodes):
    ...
    with tf.name_scope("distortion"):
      ...
      if weight is not None:
         # sq_err is (B, H, W, C). weight is (B,).
         mse_per_batch = tf.reduce_mean(sq_err, axis=[1, 2, 3])
         weighted_mse = mse_per_batch * weight
         distortion_loss = tf.reduce_mean(weighted_mse)
      else:
  ...
  def _compute_perceptual_loss(self, nodes: Nodes):
    ...
    if weight is not None:
        lpips_per_batch = self._lpips_loss(reconstruction_scaled,
                                           input_image_scaled,
                                           return_per_batch=True)
        return tf.reduce_mean(lpips_per_batch * weight)
    else:
...
class LPIPSLoss(object):
  ...
  def __call__(self, fake_image, real_image, return_per_batch=False):
