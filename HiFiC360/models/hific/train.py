...
def train(config_name, ckpt_dir, num_steps: int, auto_encoder_ckpt_dir,
          ...
          tfds_arguments: model.TFDSArguments, use_lpips_360=False,
  ...
  hific = model.HiFiC(config, helpers.ModelMode.TRAINING, lpips_weight_path,
                      ...
                      use_lpips_360, latitude_weight_type, pole_weight)
...
def parse_args(argv):
  ...
  # Arguments for 360-degree image LPIPS loss
  parser.add_argument('--use_lpips_360', action='store_true',
                      help='Use LPIPS loss adapted for 360-degree images with latitude weighting.')
  parser.add_argument('--latitude_weight_type', default='cosine',
                      choices=['cosine', 'linear', 'quadratic'],
                      help='Type of latitude weighting for 360-degree LPIPS loss.')
  parser.add_argument('--pole_weight', type=float, default=0.5,
                      help='Weight factor for polar regions (0.0 to 1.0). Lower values reduce importance of poles.')
...
def main(args):
  train(args.config, args.ckpt_dir, args.num_steps,
        ...
        helpers.parse_tfds_arguments(args), args.use_lpips_360,
        args.latitude_weight_type, args.pole_weight)
