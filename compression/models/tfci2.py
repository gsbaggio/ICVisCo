...
def main(args):
  ...
  if args.command == "compress":
    if tf.io.gfile.isdir(args.input_file):
      results_dir = args.output_file if args.output_file else "results"
      if not tf.io.gfile.exists(results_dir):
        tf.io.gfile.makedirs(results_dir)
      for filename in tf.io.gfile.listdir(args.input_file):
        if filename.lower().endswith(".png"):
          input_path = os.path.join(args.input_file, filename)
          output_path = os.path.join(results_dir, filename + ".tfci")
          print(f"Compressing {input_path} to {output_path}")
          compress(args.model, input_path, output_path,
    ...
    if tf.io.gfile.isdir(args.input_file):
      ...
      if not tf.io.gfile.exists(results_dir):
        tf.io.gfile.makedirs(results_dir)
      for filename in tf.io.gfile.listdir(args.input_file):
        if filename.lower().endswith(".tfci"):
          input_path = os.path.join(args.input_file, filename)
          output_path = os.path.join(results_dir, filename + ".png")
